...
const SelectPlace = (props: any) => {
  ...
  const [selectedPlace, selectPlace] = useState({
    place: null,
    id: null,
    country: null,
  })
  ...
  const getPlaces = async (k: string) => {
    const resultPlaces = await fetchRequest(
      `get_places.php?keyword=${k}`,
      {
        method: 'GET',
  ...
  return (
    <div>
      ...
      <ResultList
        data={result}
        showModal={(value: boolean) => showModal(value)}
        value={selectedPlace}
        selectItem={(item: any) => {
          selectPlace(item)
        ...
        <div style={modalStyle} className={classes.paper}>
          ...
          <p id="simple-modal-description">
            <AddNew
              ...
              resetCountry={() => selectCountry({ id: null, country: null })}
              setResults={(value: [String]) => setCountriesResult(value)}
